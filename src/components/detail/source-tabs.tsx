...
export function SourceTabs({ prompt, reactCode, cssCode }: SourceTabsProps) {
  ...
  return (
    <section className="relative flex min-h-[680px] flex-col overflow-hidden rounded-[24px] border border-transparent bg-[#111111] text-white shadow-[0_2px_16px_rgba(0,0,0,0.04)] transition-colors duration-200 dark:border-line dark:bg-surface" aria-label="Prompt and source code">
      <div className="border-b border-white/10 p-3">
        <div className="grid grid-cols-3 gap-1 rounded-full bg-white/8 p-1" role="tablist" aria-label="Source format">
          {tabs.map((tab) => (
            <button key={tab.id} id={`tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`panel-${tab.id}`} onClick={() => setActiveTab(tab.id)} className={`relative min-h-10 rounded-full px-3 text-xs font-bold transition-colors ${activeTab === tab.id ? "text-[#111111]" : "text-white/55 hover:text-white"}`}>
      ...
      <div aria-live="polite" aria-atomic="true" className="pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-4">
        {copied && (
          <motion.div initial={{ y: 20, opacity: 0, scale: .94 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 12, opacity: 0 }} className="rounded-full bg-surface px-5 py-3 text-sm font-bold text-foreground shadow-[0_14px_40px_rgba(0,0,0,0.16)]">
