      ...
      <div className="mt-10 flex flex-col gap-6 border-b border-line pb-10 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex rounded-full bg-lime px-3 py-1.5 text-xs font-bold uppercase tracking-[0.12em]">{categoryLabels[entry.category]}</span>
        ...
        <div className="flex max-w-md flex-wrap gap-2 lg:justify-end">
          {entry.tags.map((tag) => <span key={tag} className="rounded-full border border-line bg-white px-3 py-1.5 text-xs font-semibold text-muted">#{tag}</span>)}
        </div>
      </div>
